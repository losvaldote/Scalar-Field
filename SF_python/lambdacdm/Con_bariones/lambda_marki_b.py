...
#Adams-Bashforth 4/Moulton 4 Step Predictor/Corrector
	...
	de = []	     #dark energy
        bar = []       #bariones
        for t, p, aux1, aux2, aux3  in zip(self.t, x, z, l, b):
            if i%200 ==0:
               tiempo.append(t)
               phi.append(p)
               #dphi.append(d)
	       rad.append(aux1)
               ...
               bar.append(aux3)
            i+=1
        ax3.semilogx(tiempo, np.array(phi)*np.array(phi), 'black')   #cinetica
	...
	#ax3.semilogx(tiempo, (np.array(phi)*np.array(phi) + np.array(rad)*np.array(rad) + np.array(de)*np.array(de) + np.array(bar)*np.array(bar), 'black') #(xx+uu)
	...
	plt.legend(('$\Omega_{cdm}$', '$\Omega_{\gamma}$', '$\Omega_{\Lambda}$', '$\Omega_b$' ))
        plt.savefig('lcdm_b.pdf')
        plt.show()
