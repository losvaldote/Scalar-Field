...
#Adams-Bashforth 4/Moulton 4 Step Predictor/Corrector
	...
	rad = []     #radiacion
	de = []	     #dark energy
        for t, p, d, aux1, aux2  in zip(self.t, x, u, z, l):
            if i%200 ==0:
               tiempo.append(t)
               phi.append(p)
               dphi.append(d)
	...
	ax3.semilogx(tiempo, (np.array(phi)*np.array(phi) + np.array(dphi)*np.array(dphi)) + np.array(rad)*np.array(rad) + np.array(de)*np.array(de), 'black') #(xx+uu)
	...
	#plt.legend(('$\Omega_{dm}$', '$\Omega_{\gamma}$', '$\Omega_{\Lambda}$'))
        #plt.savefig('radiacion_dm_de_mark_iii_b.pdf')
	plt.savefig('prueba_mark_iii_b.pdf')
        plt.show()
