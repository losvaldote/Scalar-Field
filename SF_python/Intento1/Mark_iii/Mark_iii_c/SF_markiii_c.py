...
#Adams-Bashforth 4/Moulton 4 Step Predictor/Corrector
	...
	de = []	     #dark energy
        ...
        for t, p, d, aux1, aux2, aux3  in zip(self.t, x, u, z, l, b):
            #if i%200 ==0:#Resolucion de las graficas wenas
            if i%10 ==0:
               tiempo.append(t)
               phi.append(p)
               dphi.append(d)
	...
	#ax3.semilogx(tiempo, (np.array(phi)*np.array(phi) + np.array(dphi)*np.array(dphi)) + np.array(rad)*np.array(rad) + np.array(de)*np.array(de) + np.array(bar)*np.array(bar), 'black') #(xx+uu)
	...
	#ax3.semilogx(tiempo, np.array(bar)*np.array(bar), 'green')#bariones
        ax3.semilogx(tiempo, (np.array(phi)*np.array(phi) - np.array(dphi)*np.array(dphi))/(np.array(phi)*np.array(phi) + np.array(dphi)*np.array(dphi)), 'black') #ecuacion de estado
	...
	#plt.legend(('$\Omega_{dm}$', '$\Omega_{\gamma}$', '$\Omega_{\Lambda}$', '$\Omega_b$' ))
        plt.savefig('ecuacion_de_estado_mark_iii_c.pdf')
        plt.show()
